...
trait Tables {
   ...
   *  @param taskText Database column task_text SqlType(varchar), Length(2000,true), Default(None) */
  case class ItemsRow(itemId: Int, userId: Option[Int] = None, taskText: Option[String] = None)
  ...
  class Items(_tableTag: Tag) extends profile.api.Table[ItemsRow](_tableTag, "items") {
    def * = ((itemId, userId, taskText)).mapTo[ItemsRow]
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = ((Rep.Some(itemId), userId, taskText)).shaped.<>({r=>import r._; _1.map(_=> (ItemsRow.apply _).tupled((_1.get, _2, _3)))}, (_:Any) => throw new Exception("Inserting into ? projection not supported."))
    ...
    /** Database column task_text SqlType(varchar), Length(2000,true), Default(None) */
    val taskText: Rep[Option[String]] = column[Option[String]]("task_text", O.Length(2000,varying=true), O.Default(None))
